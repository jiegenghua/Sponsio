...
| [`sync-ts-mirror.sh`](sync-ts-mirror.sh) | After Python-side edits to `sponsio/contracts/` / `sponsio/prompts/`. Pushes the canonical Python copies into `ts/packages/sdk/` so the TS SDK ships the same packs / prompts. |
